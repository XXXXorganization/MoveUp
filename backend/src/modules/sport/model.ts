 ...
 * 运动记录数据模型
 */
export class SportRecordModel {
  private tableName = 'sport_records';
   ...
   * 根据 ID 查找记录
   */
  async findById(id: string): Promise<SportRecord | null> {
   ...
   * 根据用户 ID 查找记录
   */
  async findByUserId(userId: string): Promise<SportRecord[]> {
   ...
   * 查找进行中的记录
   */
  async findActiveByUserId(userId: string): Promise<SportRecord | null> {
   ...
   * 创建记录
   */
  async create(data: Omit<SportRecord, 'id' | 'created_at' | 'updated_at'>): Promise<SportRecord> {
   ...
   * 更新记录
   */
  async update(id: string, data: Partial<SportRecord>): Promise<SportRecord | null> {
    ...
    if (data.averagePace !== undefined) updateData.average_pace = data.averagePace;
    if (data.maxHeartRate !== undefined) updateData.max_heart_rate = data.maxHeartRate;
    if (data.averageHeartRate !== undefined) updateData.average_heart_rate = data.averageHeartRate;
   ...
   * 数据库记录映射到模型
   */
  private mapToModel(record: any): SportRecord {
    return {
      id: record.id,
...
export class GpsPointModel {
   ...
   * 根据记录 ID 查找轨迹点
   */
  async findByRecordId(recordId: string): Promise<GpsPoint[]> {
   ...
   * 批量插入轨迹点
   */
  async insertMany(points: Omit<GpsPoint, 'id'>[]): Promise<GpsPoint[]> {
   ...
   * 插入单个轨迹点
   */
  async insert(point: Omit<GpsPoint, 'id'>): Promise<GpsPoint> {
   ...
   * 数据库记录映射到模型
   */
  private mapToModel(point: any): GpsPoint {
    return {
      id: point.id,
...
export class HeartRateModel {
   ...
   * 根据记录 ID 查找心率数据
   */
  async findByRecordId(recordId: string): Promise<HeartRateData[]> {
   ...
   * 批量插入心率数据
   */
  async insertMany(rates: Omit<HeartRateData, 'id'>[]): Promise<HeartRateData[]> {
   ...
   * 插入单个心率数据
   */
  async insert(rate: Omit<HeartRateData, 'id'>): Promise<HeartRateData> {
    const [inserted] = await this.db(this.tableName)
      .insert({
   ...
   * 数据库记录映射到模型
   */
  private mapToModel(rate: any): HeartRateData {
    return {
      id: rate.id,
