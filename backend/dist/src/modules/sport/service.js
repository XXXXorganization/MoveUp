...
class SportService {
    constructor(repository) {
        ...
        // GPS 纠偏参数
        this.GPS_ACCURACY_THRESHOLD = 50; // 精度阈值（米），超过此值视为漂移点
        this.MAX_SPEED_THRESHOLD = 8; // 最大速度阈值（m/s），约 28.8 km/h，超过视为异常
        this.MIN_POINT_DISTANCE = 5; // 最小点间距（米），小于此值的点可能被合并
        // 卡路里计算参数
        this.BASE_MET = 8.3; // 基础代谢当量（中等强度跑步）
    }
    /**
     * 开始运动记录
     */
    async startSportRecord(userId) {
        ...
        if (activeRecord) {
            throw new errors_1.AppError('您已有一个进行中的运动记录', 400);
        }
        const record = {
            userId,
            startTime: new Date(),
    ...
    async updateSportRecord(recordId, data) {
        ...
        if (!record) {
            throw new errors_1.AppError('运动记录不存在', 404);
        }
        if (record.status !== 'active') {
            throw new errors_1.AppError('运动记录已结束，无法更新', 400);
        }
        // 处理 GPS 点（进行纠偏）
     ...
     * 批量上传 GPS 轨迹点
     */
    async batchUploadGpsPoints(recordId, points) {
        const record = await this.repository.getSportRecordById(recordId);
        if (!record) {
            throw new errors_1.AppError('运动记录不存在', 404);
        }
        if (record.status !== 'active') {
            throw new errors_1.AppError('运动记录已结束，无法上传数据', 400);
        }
        // 纠偏处理
     ...
     * 上传心率数据
     */
    async uploadHeartRateData(recordId, heartRates) {
        const record = await this.repository.getSportRecordById(recordId);
        if (!record) {
            throw new errors_1.AppError('运动记录不存在', 404);
        }
    ...
    async stopSportRecord(recordId) {
        ...
        if (record.status !== 'active') {
            throw new errors_1.AppError('运动记录已结束', 400);
        }
        ...
        if (!updatedRecord) {
            throw new errors_1.AppError('更新运动记录失败', 500);
        }
        return updatedRecord;
    }
    /**
     * 实时计算运动指标
     */
    async calculateRealTimeMetrics(recordId) {
        const record = await this.repository.getSportRecordById(recordId);
        if (!record) {
            throw new errors_1.AppError('运动记录不存在', 404);
        }
     ...
     * 最终计算运动指标（停止时使用）
     */
    async calculateFinalMetrics(recordId) {
        const record = await this.repository.getSportRecordById(recordId);
        if (!record) {
            throw new errors_1.AppError('运动记录不存在', 404);
        }
     ...
     * 统一计算所有运动指标
     */
    computeAllMetrics(record, gpsPoints, heartRates) {
        // 计算距离
     ...
     * 验证 GPS 点是否有效
     */
    isValidGpsPoint(prevPoint, currPoint) {
        // 检查精度
     ...
     * 从 GPS 点计算总距离
     */
    calculateDistanceFromGps(points) {
        if (points.length < 2)
            return 0;
        let totalDistance = 0;
        for (let i = 1; i < points.length; i++) {
     ...
     * 获取实时运动数据
     */
    async getRealTimeData(recordId) {
        const record = await this.repository.getSportRecordById(recordId);
        if (!record) {
            throw new errors_1.AppError('运动记录不存在', 404);
        }
     ...
     * 获取运动记录统计
     */
    async getSportRecordStats(recordId) {
        const record = await this.repository.getSportRecordById(recordId);
        if (!record) {
            throw new errors_1.AppError('运动记录不存在', 404);
        }
     ...
     * 计算配速分段统计
     */
    calculatePaceSegments(gpsPoints) {
        ...
        for (let i = 1; i < gpsPoints.length; i++) {
            ...
            const segmentDist = this.haversineDistance(prevPoint.latitude, prevPoint.longitude, currPoint.latitude, currPoint.longitude);
     ...
     * 处理蓝牙心率设备数据
     */
    async processBluetoothHeartRate(recordId, bluetoothData) {
        const record = await this.repository.getSportRecordById(recordId);
        if (!record) {
            throw new errors_1.AppError('运动记录不存在', 404);
        }
    ...
    async getUserSportStats(userId) {
